...
export default function NabilAcademyApp() {
  ...
  const saveToDatabase = async () => {
    ...
    // محاسبه سهم ۷۵ درصد و تقسیم بر دو
    ...
    if (!error) {
      ...
      alert("❌ خطا: احتمالا ستون name یا last_name را در دیتابیس نساخته‌اید");
        ...
        <div className="p-6 space-y-4">
          {activeTab === "register" ? (
            <>
              ...
              <input type="number" className="w-full p-4 rounded-xl border-2 text-center text-2xl font-bold text-blue-900" placeholder="مبلغ (افغانی)" onChange={e=>setForm({...form, price:Number(e.target.value)})} />
            ...
            <div className="space-y-3">
              {patients.map(p => (
                <div key={p.id} className="p-4 bg-slate-50 rounded-xl border-r-4 border-blue-900 flex justify-between">
                  <span className="font-bold text-black">{p.name} {p.last_name}</span>
                  <span className="text-blue-700 text-sm">{Number(p.dr_nabil_share).toLocaleString()} افغانی</span>
                </div>
              ))}
            </div>
          )}
